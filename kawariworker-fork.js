...
(function () {
  ...
  function clientRoutine(workerMaker) {
    var KawariWorker = function (_NativeShioriWorkerCl) {
      _inherits(KawariWorker, _NativeShioriWorkerCl);

      _createClass(KawariWorker, [{
        key: 'worker',
        get: function get() {
          return this._worker;
        }
      ...
      function KawariWorker(fs, workerErrorHandler) {
        _classCallCheck(this, KawariWorker);

        // no fs with fork
        var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(KawariWorker).call(this));
    ...
    if (typeof window !== 'undefined') {
      window.KawariWorker = KawariWorker;
      if (window.ShioriLoader) {
        window.ShioriLoader.shiories.kawari = KawariWorker;
      } else {
        throw new Error("load ShioriLoader first");
      }
    } else {
      // node.js
      module.exports = KawariWorker;
      if (typeof require !== 'undefined') {
        var ShioriLoader = require('shioriloader');
        ShioriLoader.shiories.kawari = KawariWorker;
      }
    }
  }
